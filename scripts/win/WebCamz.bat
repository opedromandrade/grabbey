...
for /f "skip=1 delims=" %%i in ('wmic path win32_videocontroller get name') do (
    echo %%i | findstr /i "NVIDIA" >nul && (set "VCODEC=h264_nvenc" & set "PRESET=p2" & goto :GpuFoundWeb)
    echo %%i | findstr /i "AMD" >nul && (set "VCODEC=h264_amf" & set "PRESET=speed" & goto :GpuFoundWeb)
    echo %%i | findstr /i "Intel" >nul && (set "VCODEC=h264_qsv" & set "PRESET=veryfast" & goto :GpuFoundWeb)
)
:GpuFoundWeb
...
ffmpeg -y -rtbufsize 1900M -thread_queue_size 1024 -f gdigrab -framerate %FRAMERATE% -video_size %SCREEN_WIDTH%x%SCREEN_HEIGHT% -i desktop -f dshow -thread_queue_size 1024 -i video="%WEBCAM_NAME%":audio="%AUDIO_DEVICE%" -filter_complex "[0:v]scale=%SCREEN_WIDTH%x%SCREEN_HEIGHT%[desktop]; [1:v]scale=%WEBCAM_RES%[webcam]; [desktop][webcam]overlay=x=W-w-50:y=H-h-50" -c:v %VCODEC% -preset %PRESET% -pix_fmt yuv420p -b:v %VIDEO_BITRATE% -c:a aac -b:a %AUDIO_BITRATE% -ar %SAMPLERATE% -async 1 -threads 4 "%OUTPUT_FILE%"
echo *** Safe and sound! ***
pause
